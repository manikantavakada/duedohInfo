...
function FileField({ label, value, onChange, hint, accept = "image/*,application/pdf" }) {
  const ref = useRef(null);
  return <div>
    <span className="label">{label}</span>
    <button type="button" onClick={() => ref.current?.click()} className="flex min-h-20 w-full overflow-hidden gap-3 rounded-xl border border-dashed border-slate-300 bg-white p-3 text-left hover:border-orange">
      <span className="grid size-11 place-items-center rounded-lg bg-orange-soft text-orange"><FileUp size={20} /></span>
      <span className="min-w-0 flex-1"><span className="block break-words text-sm font-semibold text-navy">{value?.name || "Choose a clear image or PDF"}</span><span className="mt-0.5 block text-xs text-slate-500">{hint || "JPG, PNG, or PDF up to 15 MB"}</span></span>
    </button>
    <input ref={ref} className="hidden" type="file" accept={accept} onChange={(event) => onChange(event.target.files?.[0] || null)} />
  </div>;
}
...
export default function Apply() {
  ...
  function check() {
    ...
    if (currentLabel === "Personal") {
      if (!form.dob || !form.phone.match(/^\d{10}$/) || !form.cityId || !form.qualification.trim() || !form.bio.trim() || !form.languages.length || form.experienceYears === "") return "Complete every personal detail, including qualification, experience, languages, and your introduction.";
    }
    ...
    if (currentLabel === "Vehicle") {
      ...
      if (![form.brand, form.model, form.vehicleNumber, form.seatCount, form.licenceExpiry, form.rcExpiry, form.insuranceExpiry, form.insuranceNumber].every((value) => String(value).trim()) || vehiclePhotos.some((file) => !file)) return "Complete every vehicle detail and add front, side, rear, and interior photos.";
    }
  ...
  async function next() {
    ...
    try {
      if (currentLabel === "Account") {
        ...
        await api.saveProfile({ full_name: form.fullName.trim(), email: form.email.trim(), phone: form.phone, date_of_birth: form.dob, city_id: form.cityId, qualification: form.qualification.trim(), bio: form.bio.trim(), languages: form.languages, experience_years: Number(form.experienceYears) });
        ...
        await api.saveVehicle({ vehicle_type: form.vehicleType, brand: form.brand, model: form.model, vehicle_number: form.vehicleNumber, seat_count: Number(form.seatCount), licence_expiry: form.licenceExpiry, rc_expiry: form.rcExpiry, insurance_expiry: form.insuranceExpiry, insurance_number: form.insuranceNumber });
        ...
        const documentTypes = [
          `${form.idType}_front`, `${form.idType}_back`, "selfie",
          ...(needsVehicle ? ["licence_front", "licence_back", "rc", "insurance"] : []),
        ];
        documentTypes.forEach((type) => { if (files[type]) body.append(type, files[type]); });
  ...
  return <Layout>
    <main className="mx-auto max-w-3xl px-5 py-8 sm:py-12">
      ...
      <section className="border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
        ...
        {currentLabel === "Personal" && <div className="grid gap-5 sm:grid-cols-2"><Field label="Mobile number" hint="10 digits, without +91"><input className="field" inputMode="tel" maxLength="10" value={form.phone} onChange={(e) => set("phone", e.target.value.replace(/\D/g, ""))} placeholder="9876543210" /></Field><Field label="Date of birth"><input className="field" type="date" value={form.dob} onChange={(e) => set("dob", e.target.value)} /></Field><Field label="Home city"><select className="field" value={form.cityId} disabled={loadingLookups} onChange={(e) => set("cityId", e.target.value)}><option value="">Select a Duedoh city</option>{cities.map((city) => <option key={city.id} value={city.id}>{city.name}{city.state ? `, ${city.state}` : ""}</option>)}</select></Field><Field label="Qualification"><input className="field" value={form.qualification} onChange={(e) => set("qualification", e.target.value)} placeholder="e.g. Graduate" /></Field><Field label="Hosting experience"><input className="field" type="number" min="0" max="60" value={form.experienceYears} onChange={(e) => set("experienceYears", e.target.value)} /></Field><div className="sm:col-span-2"><Field label="Languages you speak"><div className="flex flex-wrap gap-2">{languages.map((language) => <Toggle key={language} active={form.languages.includes(language)} onClick={() => set("languages", form.languages.includes(language) ? form.languages.filter((item) => item !== language) : [...form.languages, language])}>{language}</Toggle>)}</div></Field></div><div className="sm:col-span-2"><Field label="About you"><textarea className="field min-h-28" maxLength="300" value={form.bio} onChange={(e) => set("bio", e.target.value)} placeholder="Your local knowledge and how you like to help travellers." /></Field></div></div>}
